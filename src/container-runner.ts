...
function buildVolumeMounts(
  ...
  if (isMain) {
    ...
    if (fs.existsSync(globalDir)) {
      ...
      includeDirectories.push('/workspace/global');
    }
  }

  const validatedMounts = group.containerConfig?.additionalMounts
  ...
  for (const mount of validatedMounts) {
    ...
    try {
      if (fs.statSync(mount.hostPath).isDirectory()) {
        includeDirectories.push(mount.containerPath);
      }
    } catch {
      // Ignore missing/broken additional mounts here — container runtime will fail later.
    }
  }

  // Per-group Qwen home directory (isolated from other groups)
  // Each group gets their own .qwen/ to prevent cross-group session access.
  ...
  writeQwenSettings(
    path.join(groupSessionsDir, 'settings.json'),
    includeDirectories,
  );
  mounts.push({
...
function buildContainerArgs(mounts: VolumeMount[], containerName: string): string[] {
  ...
  // Pass host timezone so the container agent sees the same local time as the host.
